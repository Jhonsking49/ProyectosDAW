...
// EJERCICIO 1: SACAR Y GUARDAR EN UN ARRAY TODOS LOS USERNAMES DE LOS USUARIOS

const usernames = [];

users.forEach((user) => {
    if (user.login && user.login.username !== undefined) {
...
// sacar  firstname y lastname de los usuarios que no sean .com

const filteredUsers = users.filter((user) => !user.email.endsWith(".com"));
const result = filteredUsers.map((user) => ({ firstname: user.firstname, lastname: user.lastname }));

console.log(result);

//Crear una estructura de tipo MAP que guarde el firtsname y el lastname con su email
